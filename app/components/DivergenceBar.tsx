...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="bg-[#1c2128] border border-[#30363d] rounded-lg p-3 text-xs shadow-xl">
      ...
      <p className="text-gray-400 mb-2">{d.league} · {d.sport}</p>
      <div className="space-y-1 font-mono">
        ...
        <p>
          ...
          <span className={d.diffPct >= 0 ? "text-[#3fb950]" : "text-[#f85149]"}>
            {d.diffPct >= 0 ? "+" : ""}{d.diffPct.toFixed(2)} pp
          </span>
        </p>
      </div>
    </div>
...
export default function DivergenceBar({ events }: Props) {
  ...
  const shortLabel = (ev: string) => ev.split(" vs ")[0].split(" ").at(-1) ?? ev;
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 10 }}>
            ...
            <ReferenceLine
              ...
              strokeDasharray="4 4"
              strokeWidth={1}
              label={{ value: "+5%", fill: "#d29922", fontSize: 10 }}
            />
            <ReferenceLine
              y={-5}
              stroke="#d29922"
