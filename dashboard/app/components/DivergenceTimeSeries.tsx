...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-[#1c2128] border border-[#30363d] rounded-lg p-3 text-xs shadow-xl min-w-[180px]">
      <p className="font-mono text-gray-400 mb-2">{label}</p>
      {payload.map((p: any) => (
        <div key={p.dataKey} className="flex justify-between gap-3 font-mono">
          <span style={{ color: p.color }}>{p.name}</span>
          <span className={p.value >= 0 ? "text-[#3fb950]" : "text-[#f85149]"}>
            {p.value >= 0 ? "+" : ""}{(p.value * 100).toFixed(2)} pp
          </span>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="bg-surface-card border border-surface-border rounded-lg overflow-hidden">
      <div className="px-4 py-3 border-b border-surface-border">
        ...
        <p className="text-xs text-gray-500 mt-0.5">Hourly snapshot — market prob minus sportsbook implied probability</p>
      </div>
      ...
      <div className="flex flex-wrap gap-2 px-4 pt-3">
        {EVENTS.map((ev) => (
          ...
          >
            <span className="w-2.5 h-2.5 rounded-full inline-block" style={{ backgroundColor: EVENT_COLORS[ev.id] }} />
